...
#		die "objectnumbers are unnecessary when using -tags. Please use the -net switch with a coappearance graph input or exclude $objn_file!\n" unless $use_coappearances;
                ...
                $threshold = $ARGV[$i + 1];
		die $help_msg unless $threshold =~ /^\d+(\.\d+)?$/ and $threshold > -0.0000000000001 and $threshold < 1.0000000000001;
#                die "threshold should be a real number between 0 and 1!\n" unless $threshold =~ /^\d+(\.\d+)?$/ and $threshold > -0.0000000000001 and $threshold < 1.0000000000001;
        }
	elsif ($ARGV[$i] eq '-dag'){
		$use_origDAG = 1;
		$dag_file = $ARGV[$i + 1];
...
for $label (keys %reduced_directed_ll_nb){
	$i++;
	%_ = %{$zscore{$label}};
...
for $label (keys %ll_nb){
	...
	unless (exists $reconstr_parent{$label} and $reconstr_parent{$label} and keys %{$reconstr_parent{$label}}){
		$roots{$label} = undef;
		if ($max_h < $h{$label} - 0.0000000000001){
			$max_h = $h{$label};
			$global_root = $label;
		}
	}
...
print STDERR "number of new links is $_\n";




for $root (keys %suggested_parent){
	$all += 1;
	if ($use_origDAG){
		...
		elsif (exists $child{$root} and exists $child{$root}{$suggested_parent{$root}}){
			$inverted += 1;
		}
...
die "total number of found edges = $all\n\noutput written to $fout\n\n";
