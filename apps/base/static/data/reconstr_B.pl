...
my ($label, $label2, $label3, $i, $j, $k, $fout, $fout2, $fout3, $parent, $flag, $O, $current, $next, $exact, $lineage, $near, $mu, $sigma, $threshold, $max_w, $max_z, $max_d, $c, $all, $inverted, $differentbranches, $use_origDAG, $use_annots, $annots_file, $use_coappearances, $coapps_file, $objn_file, $dag_file, $comment, $help_msg, $i0, $component, $s_avg, $comp_one, $multi, $i_max);
my (@temp, @currents, @temp2, @componentlist);
my (%ll_nb, %nb, %child, %parent, %centrality, %strength, %objectnumber, %temp, %nexts, %visited, %score, %ancestors, %siblings, %zscore, %temp2, %ll_nb_filtered, %descendantsvotes, %child_reconstr, %sum, %sum2, %component, %lambda, %lambda2, %parents, %child_reconstr_bydescvotes, %parent_reconstr, %ancs);


$threshold = 10;       # default value
$comment = '#';        # comment line indicator
$help_msg = "\nusage:\n\nperl reconstr_B.pl [arguments]\n\nmandatory arguments:\n\t-tags OBJECTS_WITH_TAGS_FILE (mutually exclusive with -net)\n\t-net  WEIGHTED_TAG_COAPPEARANCES_FILE (mutually exclusive with -tags)\n\t-objn OBJECTNUMBERS_FILE (only when using -net)\n\noptional arguments:\n\t-dag ORIGINAL_DAG_FILE\n\t-z   Z-SCORE_THRESHOLD_VAUE (a nonnegative real number; default is 10)\n\t-p   multi (set multi-parent mode)\n\t-c   merge (force 1 connected component)\n\t-o   OUTPUT_FILE\n\ncomment lines in the input files should begin with # (whitespaces before the # are allowed)\n\nexamples:\nperl reconstr_B.pl -tags tagged_objects_file.txt\nperl reconstr_B.pl -net tag_coappearances.txt -objn objectnumbers.txt -o output.txt -z 8.67 -dag dag_file.txt\n\n";
...
#		die "$threshold is not a valid z-score threshold!\n" unless $threshold =~ /^\d+(\.\d+)?$/;
	...
	elsif ($ARGV[$i] eq '-p' and $ARGV[$i + 1] eq 'multi'){
		$multi = 1;
	}
	elsif ($ARGV[$i] eq '-c' and $ARGV[$i + 1] eq 'merge'){
		$comp_one = 1;
	}
	else {
		die $help_msg;
#		die "unknown input parameter: $ARGV[$i]!\ntype perl reconstr_B.pl for help!\n";
	}
}
...
unless ($fout){
	if ($use_annots){
        ...
        if ($fout =~ /\./){
                $fout =~ s/\.[^\.]+$/_tag_hierarchy_B_multiparent.txt/;
        }
        else {
                $fout .= "_tag_hierarchy_B_multiparent.txt";
        }
}
$fout2 = $fout;
$fout2 =~ s/tag_hierarchy_B_multiparent\.txt/statistics_B_multiparent.txt/;
$fout3 = $fout;
$fout3 =~ s/tag_hierarchy_B_multiparent\.txt/centralities_B_multiparent.txt/;
if (-e $fout){
	$i = 2;
	do {
		if ($fout =~ /B_multiparent.txt$/){
			$fout  =~ s/B_multiparent\.txt$/B_multiparent_$i.txt/;
			$fout2 =~ s/B_multiparent\.txt$/B_multiparent_$i.txt/;
		}
		else {
			$fout  =~ s/B_multiparent_\d+\.txt$/B_multiparent_$i.txt/;
			$fout2 =~ s/B_multiparent_\d+\.txt$/B_multiparent_$i.txt/;
		}
		$i++;
	} while (-e $fout);
}
...
print STDERR "set to single-parent mode\n" unless $multi;
print OUT2   "set to single-parent mode\n" unless $multi;
print STDERR "set to multi-parent mode\n" if $multi;
print OUT2   "set to multi-parent mode\n" if $multi;
print STDERR "force 1 connected component\n" if $comp_one;
print OUT2   "force 1 connected component\n" if $comp_one;
print STDERR "allow multiple connected components\n" unless $comp_one;
print OUT2   "allow multiple connected components\n" unless $comp_one;
...
for $label (keys %ll_nb){
	for $label2 (keys %{$ll_nb{$label}}){
		...
		if ($sigma){
			$zscore{$label}{$label2} = ($ll_nb{$label}{$label2} - $mu) / $sigma;
		}
...
if (-e $fout3){
	...
	do {
		...
		while (%nexts){
			...
			for $current (@currents){
				if (exists $ll_nb_filtered{$current}){
	...
	for $label (@_){
		...
		$sum{$component{$label}} += $strength{$label};
	}
	...
	for $label (reverse sort centralitysort keys %ll_nb){
...
for $label (sort centralitysort keys %ll_nb){
	...
	undef %score;
	for ($i = 0; $i < 2 and not %score; $i++){	# if the first round is unsuccesful (no parent-candidates), for the second run ($i == 1) the z-score threshold is neglected - we enforce a single connected component
		undef %nexts;
		$nexts{$label} = undef;
		do {
			@currents = keys %nexts;
			undef %nexts;
			for $current (@currents){
				for $next (keys %{$child_reconstr_bydescvotes{$current}}){
					for $label2 (keys %{$ll_nb{$next}}){
						if ($i == 0){
							$descendantsvotes{$label2} += $zscore{$label2}{$next} if $label2 ne $label and $label2 ne $next and exists $ll_nb{$label}{$label2} and $centrality{$label2} > $centrality{$label} and ($zscore{$label}{$label2} >= $threshold or $ll_nb{$label}{$label2} / $objectnumber{$label} > 0.5) and ($zscore{$next}{$label2} >= $threshold or $ll_nb{$next}{$label2} / $objectnumber{$next} > 0.5);
						}
					}
					$nexts{$next} = undef; 
				}
		...
		for $label2 (keys %{$ll_nb{$label}}){
			if ($label2 ne $label and $centrality{$label2} > $centrality{$label} and ($i or ($zscore{$label}{$label2} >= $threshold or $ll_nb{$label}{$label2} / $objectnumber{$label} > 0.5))){
	...
	if (%score){
		...
		$parents{$parent} = undef;
		if ($multi and $i < 2){			# multiple parents are switched off for the z-score-thresholdless "find any possible parent" mode
							# IMPORTANT : should multiple parents allowed for the z-score-thresholdless case, for the second, third, etc. parents it should be verified that no directed loops are formed, or skip that candidate parent
			$flag = 1;
			@_ = sort {$zscore{$label}{$b} <=> $zscore{$label}{$a}} keys %score;
			for ($i = 0; $i < @_ and $flag; $i++){
				if ($zscore{$label}{$_[$i]} >= $zscore{$label}{$parent} - 0.000000000000001){
					$parents{$_[$i]} = undef;
				}
				else {
					$flag = 0;
	...
	if (%parents){
		$child_reconstr_bydescvotes{$parent}{$label} = undef;
		for $parent (sort {$centrality{$b} <=> $centrality{$a}} keys %parents){
# 			$child_reconstr{$parent}{$label} = undef;
# 			$parent_reconstr{$label}{$parent} = undef;
			$all += 1;
			if ($use_origDAG){
...
$_ = (keys %ll_nb) - 1;
print STDERR "\ntotal number of found edges = $all (max possible for a spanning tree is $_)\n\noutput written to $fout\n\n";
print OUT2   "\ntotal number of found edges = $all (max possible for a spanning tree is $_)\n";
close OUT2;
